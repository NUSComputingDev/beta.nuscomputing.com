	...
	def perform(*args)
		...
		if DateTime.now > round.end
			...
			ballots.each do |ballot|
                next if LockerAllocation.find_by(user_id: ballot.user.id, status: 0)
			...
			# Send mail
			allocated_users.each { |user, alloc| LockerNotifier.successful_ballot(user, round, alloc).deliver_later }
			unallocated_users.each { |user| LockerNotifier.unsuccessful_ballot(user, round).deliver_later }
		end
	end
end
